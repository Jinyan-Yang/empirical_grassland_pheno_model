...
for (i in seq_along(species.vec[1:9])) {
  ...
  with(dat.both.df[month(dat.both.df$Date) %in% 8:11,],
       mean(GCC.mean.drt,na.rm = T) /  mean(GCC.mean.con,na.rm = T))

  with(dat.both.df[month(dat.both.df$Date) %in% 8:11,],
       mean(cover.pred.drt,na.rm = T) /  mean(cover.pred.con,na.rm = T))
  # make plot
  dig.df <- dat.both.df
  with(dig.df,plot(GCC.mean.con~Date,
                   main = species.vec[i],ylim=c(0,1),
                   xaxt='n',xlab=''))
  with(dig.df,points(GCC.mean.drt~Date,pch=16,col='red'))
  # 
